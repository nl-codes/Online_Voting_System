...
const LoginPage = () => {
    ...
    const handleLogin = async (e) => {
        ...
        const response = await fetch("http://localhost:5000/user_login", {
            ...
            body: JSON.stringify({ email: email, pass_word: password }),
        });
            ...
            {user ? (
                ...
                <div className="min-h-screen flex flex-row gap-20 items-center justify-center bg-[#c791d4] text-[#512C59] font-poppins p-4">
                    ...
                    <div className="border-4 border-[#512C59] rounded-3xl p-8 w-96 shadow-xl bg-white flex flex-col items-center">
                        <h2 className="text-3xl font-bold mb-6 text-[#564181]">Login</h2>

                        <form className="flex flex-col items-center w-full">
                            <label className="self-start text-[#512C59] font-medium">Email:</label>
                            <input
                                maxLength="30"
                                minLength="5"
                                required
                            ...
                            <label className="self-start text-[#512C59] font-medium">Password:</label>
                            <input
                                maxLength="30"
                                minLength="5"
                                required
                            ...
                            <button onClick={handleLogin} className="w-full font-bold text-lg py-2 bg-[#564181] text-white rounded-full mt-4 transition-all duration-300 hover:bg-[#512C59] hover:text-white">
                                Login
                            </button>
                        </form>
                        ...
                        <p className="mt-4 text-[#512C59]">
                            ...
                            <Link to="/signup" className="text-[#5D76EE] font-semibold hover:underline ml-1">Sign Up now</Link>
                        </p>
                    </div>
                </div>
